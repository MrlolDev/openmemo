...
import { getMemoryTagsArray, hasMemoryTags } from '../utils/memoryHelpers';
    ...
    <div className="group relative p-3 bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg transition-all duration-300 hover:bg-white/8 hover:border-[#A8FF00]/30 animate-fade-in">
      ...
      <div className="relative">
        ...
        <div className="flex justify-between items-center mb-2">
          <Tooltip content={`Category: ${memory.category}`} position="top">
          ...
          <div className="flex items-center gap-0.5">
            {onFindSimilar && (
              <Tooltip content="Find similar memories" position="left">
                <button
            ...
            <Tooltip content="Delete this memory" position="left">
              <button
                onClick={onDelete}
        ...
        <Tooltip 
          content={memory.content.length > 100 ? memory.content : ''} 
          position="top"
        >
          ...
          {hasMemoryTags(memory.tags) && (
            <div className="flex gap-1">
              {getMemoryTagsArray(memory.tags).slice(0, 1).map((tag, index) => (
                <Tooltip key={index} content={`Tag: ${tag}`} position="bottom">
